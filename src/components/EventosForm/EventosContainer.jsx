...
function EventosContainer() {
  ...
  const findFrequenciasByAluno = async (userId) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:8080/api/frequencias/aluno/${userId}`);
  ...
  const findFrequenciasByEvento = async (eventoID) => {
    try {
      const response = await axios.get(`http://localhost:8080/api/frequencias/evento/${eventoID}`);
      console.log(response)
  ...
  const registrarFrequencia = async (eventoId, alunoId) => {
    setLoading(true);
    try {
      await axios.post(`http://localhost:8080/api/frequencias/registrar`, null, {
        params: {
          eventoId,
          alunoId
        }
  ...
  const gerarCertificado = async (eventoId) => {
    setLoading(true);
    try {
      const response = await axios.get(`http://localhost:8080/api/frequencias/certificado?eventoId=${eventoId}&alunoId=${user.id}`, {
        responseType: 'blob',
      });
  ...
  const handleRegistrarFrequencia = async (eventoId, alunoId, username) => {
    ...
    if (!frequencias.some(frequencia => frequencia.nomeParticipante === username)) {
      setLoading(false);
  ...
  return {
    states: { eventos, titulo, descricao, localizacao, dataInicio, dataFim, tipo, user, editingEvento, searchTitulo, filterInscritos, error, frequencias, loading },
    functions: {
      fetchEventos,
      searchEventos,
      createEvento,
