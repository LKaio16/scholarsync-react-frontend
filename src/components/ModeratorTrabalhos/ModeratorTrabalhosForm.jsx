  ...
  return (
    <div className="moderator-dashboard">
      ...
      <div style={{ padding: "50px" }}>
        ...
        <div className="form-trabalho">
          <div>
            <label htmlFor="titulo">Título:</label>
            <input
              type="text"
              id="titulo"
              value={states.novoTrabalhoTitulo}
              onChange={(e) => functions.setNovoTrabalhoTitulo(e.target.value)}
            />
          </div>
          <div>
            ...
            <textarea
              id="descricao"
              value={states.novoTrabalhoDescricao}
              onChange={(e) =>
                functions.setNovoTrabalhoDescricao(e.target.value)
              }
            />
          </div>
          <button
            className="button-LancarTrabalho"
            onClick={functions.handleLancarNovoTrabalho}
          >
        ...
        <div className="trabalhos-list">
          {states.trabalhos.map((trabalho) => (
            <div key={trabalho.id} className="trabalho-card">
              ...
              <button
                onClick={() => functions.handleVerSolucoes(trabalho.id)}
                ...
                {states.solucoes.map((solucao) => (
                  <tr key={solucao.id}>
                    ...
                    <td>
                      <button
                        onClick={() =>
                          functions.handleDownloadSolucao(solucao.id)
                        }
                      >
                        Baixar Solução
                      </button>
                      <button
                        onClick={() => {
                          functions.setSelectedSolucao(solucao);
                          functions.handleOpenModal();
                        }}
                      >
                        Avaliar
                      </button>
        ...
        >
          ...
          <div>
            ...
            <input
              type="text"
              id="avaliacaoNota"
              value={states.avaliacaoNota}
              onChange={(e) => functions.setAvaliacaoNota(e.target.value)}
            />
          </div>
          <div>
            ...
            <textarea
              ...
              value={states.avaliacaoComentario}
              onChange={(e) => functions.setAvaliacaoComentario(e.target.value)}
            />
          </div>
          <button
            onClick={() =>
              functions.handleAvaliarSolucao(
                states.selectedSolucao.id,
                states.avaliacaoComentario,
                states.avaliacaoNota
              )
            }
          >
            Enviar Avaliação
